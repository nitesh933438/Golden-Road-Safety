...
export function Footer() {
  ...
  return (
    <footer 
      id="app-footer" 
      className="border-t border-surface-800 bg-[#070D19] text-surface-200 pt-16 pb-[calc(3rem+56px+env(safe-area-inset-bottom))] lg:pb-12 px-4 sm:px-8 mt-auto relative z-20"
      ...
      <div className="max-w-7xl mx-auto space-y-12">
        ...
        <div className="hidden md:grid grid-cols-2 lg:grid-cols-5 gap-8 lg:gap-12 pb-4">
          ...
          <div className="col-span-2 space-y-4">
            <div className="flex items-center gap-2.5">
              ...
              <div>
                ...
                <span className="text-[10px] font-black text-amber-500 tracking-widest block uppercase">
                  AI ROAD SAFETY & SOS
                </span>
              </div>
            </div>
            
            <p className="text-xs text-surface-400 leading-relaxed max-w-sm font-medium">
              Helping citizens report road hazards, request emergency assistance, and connect with nearby safety resources.
            </p>

            <div className="pt-2">
              <p className="text-[10px] text-surface-500 leading-relaxed max-w-sm italic border-l-2 border-surface-800 pl-3">
          ...
          <div className="space-y-4">
            <h4 className="text-xs font-black uppercase text-surface-400 tracking-wider">
              Quick Links
            </h4>
              ...
              <ul className="space-y-3">
                {quickLinks.map((link) => (
                  <li key={link.label}>
                    <Link 
                      to={link.to} 
                      className="text-xs font-semibold text-surface-300 hover:text-amber-500 transition-colors focus:outline-none focus:ring-1 focus:ring-amber-500 focus:ring-offset-1 focus:ring-offset-surface-950 rounded px-1 py-0.5"
                    >
                      {link.label}
                    </Link>
                  </li>
          ...
          <div className="space-y-4">
            <h4 className="text-xs font-black uppercase text-surface-400 tracking-wider flex items-center gap-1.5">
              Account 
              <span className="text-[10px] text-amber-500 font-black lowercase bg-amber-500/10 px-1.5 py-0.5 rounded">
                {userRole}
              </span>
            </h4>
              ...
              <ul className="space-y-3">
                {accountLinks.map((link) => (
                  <li key={link.label}>
                    <Link 
                      to={link.to} 
                      className="text-xs font-semibold text-surface-300 hover:text-amber-500 transition-colors flex items-center gap-2 focus:outline-none focus:ring-1 focus:ring-amber-500 focus:ring-offset-1 focus:ring-offset-surface-950 rounded px-1 py-0.5"
                    >
        ...
        <div className="block md:hidden space-y-4">
          ...
          <div className="border-b border-surface-800 pb-2">
            <button
              ...
              className="w-full flex items-center justify-between py-2 text-xs font-black uppercase text-surface-300 tracking-wider focus:outline-none min-h-[44px]"
            >
              ...
              <ul className="py-2 pl-2 space-y-3">
                {quickLinks.map((link) => (
                  <li key={link.label}>
                    <Link 
                      to={link.to} 
                      className="text-xs font-semibold text-surface-400 hover:text-amber-500 transition-colors block min-h-[36px] flex items-center"
                    >
                      {link.label}
                    </Link>
                  </li>
          ...
          <div className="border-b border-surface-800 pb-2">
            ...
            >
              <span className="flex items-center gap-1.5">
                Account 
                <span className="text-[10px] text-amber-500 font-bold lowercase bg-amber-500/10 px-1 py-0.5 rounded">
                  {userRole}
                </span>
              </span>
              ...
              <ul className="py-2 pl-2 space-y-3">
                {accountLinks.map((link) => (
                  <li key={link.label}>
                    <Link 
                      to={link.to} 
                      className="text-xs font-semibold text-surface-400 hover:text-amber-500 transition-colors flex items-center gap-2 min-h-[36px]"
                    >
        ...
        <div className="hidden md:flex flex-wrap items-center gap-x-6 gap-y-2 py-4 border-b border-surface-800/60 text-xs">
          ...
          {legalLinks.map((link) => (
            <Link 
              key={link.label} 
              to={link.to} 
              className="font-semibold text-surface-400 hover:text-amber-500 transition-colors focus:outline-none focus:underline"
            >
              {link.label}
            </Link>
          ))}
        ...
        <div className="space-y-6 pt-2">
          ...
          <div className="bg-[#0e172a]/60 border border-surface-800/80 rounded-2xl p-4 flex flex-col sm:flex-row items-start gap-3.5 text-xs">
            <AlertTriangle className="w-4 h-4 text-amber-500 shrink-0 mt-0.5" />
            <p className="text-surface-400 leading-normal font-medium text-[11px] sm:text-xs">
              <span className="font-extrabold text-amber-500 block sm:inline mr-1">Official Safety Disclaimer:</span>
          ...
          <div className="flex flex-col md:flex-row items-center justify-between border-t border-surface-800/60 pt-6 text-xs text-surface-500 gap-4">
            
            <div className="text-center md:text-left space-y-1">
              <p className="font-extrabold text-surface-300">
            ...
            <div className="flex items-center gap-4 flex-wrap justify-center text-[10px]">
              ...
              <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-surface-900 border border-surface-800 font-extrabold tracking-wide uppercase select-none">
                {systemStatus === "Checking" && (
                  <>
                    <span className="w-1.5 h-1.5 rounded-full bg-surface-500 animate-pulse"></span>
                    <span className="text-surface-400">Status Check...</span>
                  </>
                )}
                ...
                {systemStatus === "System status unavailable" && (
                  <>
                    ...
                    <span className="text-surface-400">System status unavailable</span>
                  </>
                )}
              </div>

              {/* Package version read from actual package.json */}
              <span className="font-black text-surface-600 px-2 py-1 rounded bg-surface-900 border border-surface-800">
                v{appVersion}
              </span>
            </div>
